...
$(document).ready(function() {
	var Minimal = {};
	var TOPIC_PREFIX = '/topic/backbonestomp';
	
	var onconnect = function() {
		window.stomp.subscribe(TOPIC_PREFIX+'.ping', function() {
			return;
		})
		...
		Minimal.Todo = Backbone.Model.extend({
			...
			initialize : function() {
				 ...
				 */
				if (!this.noIoBind) {
					this.stompBind('update', this.serverChange, this);
					this.stompBind('delete', this.serverDelete, this);
				}
			},
		...
		Minimal.Todos = Backbone.Collection.extend({
			model : Minimal.Todo,
			url : TOPIC_PREFIX+'.todos',
			socket : window.stomp,
			initialize : function() {
				_.bindAll(this, 'serverCreate', 'collectionCleanup');
				this.stompBind('create', this.serverCreate, this);
			},
		...
		Minimal.TodoList = Backbone.View.extend({
			...
			removeTodo : function(todo) {
				var self = this, width = this.$('#' + todo.id).outerWidth();
		...
		Minimal.TodoListItem = Backbone.View.extend({
			...
			render : function() {
				...
				var templated = '<div title="' + json.title + '" completed="' + json.completed + '" id="' + json.id + '" class="todo"><h3 class="title">' + json.title + '</h3><div class="button complete"><div class="status"></div><div class="check">✓</div><div class="text">Complete</div></div><div class="button delete">Delete</div></div>';
		...
		Minimal.TodoListForm = Backbone.View.extend({
			...
			render : function() {
				$(this.el).html('<div id="TodoForm"><div class="fieldset field-text" id="TodoInput"><label>Create a new Todo</label><div class="fieldwrap"><input type="text" value="" name="TodoInput"></div></div><div class="button" id="add">Add</div></div>');
				return this;
			},
			addTodo : function() {
	...
	var stomp = window.stomp = Stomp.client("ws://localhost:61614");
	stomp.clientId = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
	...
	stomp.connect("backbonestomp", "backbonestomp", onconnect);
